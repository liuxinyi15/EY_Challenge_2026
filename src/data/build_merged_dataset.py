...
功能：
  1. 加载 water_quality_training_dataset.csv、两份 Landsat 源（Official + API）、
     terraclimate_features_training.csv（已有 pet，动态拉取 ppt/tmax/tmin/q）。
...
WQ_FILE = os.path.join(DATA_DIR, "water_quality_training_dataset.csv")
LANDSAT_OFFICIAL = os.path.join(GENERAL_DIR, "landsat_features_training.csv")
LANDSAT_API = os.path.join(ROOT, "data", "processed", "landsat_api_training.csv")
TERRA_FILE = os.path.join(GENERAL_DIR, "terraclimate_features_training.csv")
MERGED_OUT = os.path.join(OUTPUT_DIR, "merged_training_data_clean.csv")
